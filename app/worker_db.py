...
from models import UsersTelegram, ChatGpt, SavedQuestion, Statistic, engine
...
def get_settings(id):
    read_data = session.query(ChatGpt)\
        .filter(ChatGpt.id == id)\
            .first()
    if read_data:
        return read_data.id, read_data.temp_chat, read_data.lang_chat, read_data.count_req_chat, read_data.used_token_chat, read_data.limit_token_chat, read_data.the_gap, read_data.model_id, read_data.money_user, read_data.total_spent_money or None
    else:
        return None, None
...
def add_update_settings(id, temp_chat=None, lang_chat=None, count_req_chat=None, used_token_chat=None, limit_token_chat=None, the_gap=None, model_id=None, money_user=None, total_spent_money=None):
...
def admin_static_db():
    ...
    for user, chat_gpt in all_data:
        user_data_list.append((user.user_username, user.user_first_name, chat_gpt.used_token_chat, chat_gpt.limit_token_chat))
